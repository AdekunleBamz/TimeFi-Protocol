...
Read-only check only — does not move funds.
Treat it as a preflight hint rather than a guarantee for subsequent write calls.
...
Useful in frontends before enabling owner-only actions.
Calls for unknown vault IDs return `ERR_NOT_FOUND`.
...
In this contract variant, non-contract principals always resolve to `false`.
