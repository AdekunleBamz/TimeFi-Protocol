...
(define-read-only (can-execute (id uint))
  (match (map-get? proposals {id: id})
    p
      (let (
        (tvl-result (contract-call? .timefi-vault-v-A1 get-tvl))
...
(define-public (execute-proposal (id uint))
  (let (
    ...
    (tvl-result (unwrap! (contract-call? .timefi-vault-v-A1 get-tvl) ERR_UNAUTHORIZED))
