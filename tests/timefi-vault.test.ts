...
describe("TimeFi Vault - Basic Operations", () => {
  describe("create-vault", () => {
    it("should create a vault with valid parameters", () => {
      ...
      const { result } = simnet.callPublicFn(CONTRACT_NAME, "create-vault", [Cl.uint(amount), Cl.uint(lockBlocks)], wallet1);
      expect(result).toBeOk(Cl.uint(1));
    });
    it("should fail with amount below minimum", () => {
      const { result } = simnet.callPublicFn(CONTRACT_NAME, "create-vault", [Cl.uint(5000), Cl.uint(144)], wallet1);
      expect(result).toBeErr(Cl.uint(103));
    });
    it("should fail with lock below minimum", () => {
      const { result } = simnet.callPublicFn(CONTRACT_NAME, "create-vault", [Cl.uint(1_000_000), Cl.uint(5)], wallet1);
      expect(result).toBeErr(Cl.uint(104));
    });
    it("should fail with lock above maximum", () => {
      const { result } = simnet.callPublicFn(CONTRACT_NAME, "create-vault", [Cl.uint(1_000_000), Cl.uint(52_561)], wallet1);
      expect(result).toBeErr(Cl.uint(104));
    });
  });
  ...
  describe("get-vault", () => {
    it("should return vault details", () => {
      simnet.callPublicFn(CONTRACT_NAME, "create-vault", [Cl.uint(1_000_000), Cl.uint(144)], wallet1);
