...
(define-read-only (calculate-emergency-payout (vault-id uint))
  (let (
    (vault-result (contract-call? .timefi-vault-v-A1 get-vault vault-id))
  )
    (match vault-result
      vault
        (let (
...
;; -------------------------------------------------------
;; PUBLIC: EMERGENCY WITHDRAW (with penalty)
;; -------------------------------------------------------
...
(define-public (emergency-withdraw (vault-id uint))
  (let (
    ...
    (treasury (unwrap! (contract-call? .timefi-vault-v-A1 get-treasury) ERR_UNAUTHORIZED))
  )
    ...
    ;; In emergency mode, no penalty; otherwise apply penalty
    (if (var-get emergency-mode)
      ;; Emergency mode: full withdrawal, no penalty
      (begin
        ;; This would need the vault contract to support this
        ;; For now, we track the intent
        (map-set user-emergency-cooldown {user: tx-sender}
          {last-emergency: tenure-height})
        (print {event: "emergency-withdraw-request", vault-id: vault-id, user: tx-sender, amount: amount, penalty: u0, mode: "emergency"})
        (ok {payout: amount, penalty: u0}))
      ;; Normal mode: 25% penalty
      (begin
        (map-set user-emergency-cooldown {user: tx-sender}
          {last-emergency: tenure-height})
        (var-set penalty-collected (+ (var-get penalty-collected) penalty))
        (print {event: "emergency-withdraw-request", vault-id: vault-id, user: tx-sender, amount: amount, penalty: penalty, payout: payout, mode: "penalty"})
        (ok {payout: payout, penalty: penalty})))))
